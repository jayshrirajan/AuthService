package com.msys.authservice.service;
...
import com.msys.authservice.util.KeyCloakUtilService;
...
private RedisTemplate redisTemplate;
    public Flux<UserDto> getAllUser() {
        try {
            ...
            Flux<UserDto> users = webClientConfig.webClientAuthService()
                    .get().uri(baseUrl + keycloakBaseUrl + realmsName + "/users")
                    .headers(httpHeaders -> httpHeaders.setBearerAuth(token.block()))
                    .retrieve().bodyToFlux(UserDto.class).cache();
            return users;
        } catch (Exception e) {
            log.error("err:"+e);
    ...
    public Flux<UserDto> getUserByUserName(String username) {
        try {
            Mono<String> token = keyCloakUtil.getAccessToken();
            Flux<UserDto> user = webClientConfig.webClientAuthService().get().uri(u -> u.path(keycloakBaseUrl + realmsName + "/users").queryParam("username", username).queryParam("exact", true).build())
                    .headers(httpHeaders -> httpHeaders.setBearerAuth(token.block()))
                    .retrieve().bodyToFlux(UserDto.class).cache();
            log.debug("getUserByUserName {}", user);
             return user;
        } catch (Exception e) {
            log.error("err getUserByUserName:: "+e);
            throw new RuntimeException(e.getMessage());
        }
    }
    ...
    public UserDto updateUser(UserDto userDto, String id) {
        Mono<String> token = keyCloakUtil.getAccessToken();
        UserDto user = webClientConfig.webClientAuthService()
                .put().uri(baseUrl + keycloakBaseUrl + realmsName + "/users/" + id)
    ...
    public UserDto deleteUser(String id) {
        ...
        Mono<String> token = keyCloakUtil.getAccessToken();
        UserDto user = webClientConfig.webClientAuthService().delete()
                .uri(baseUrl + keycloakBaseUrl + realmsName + "/users/" + id)
                .headers(httpHeaders -> httpHeaders.setBearerAuth(token.block()))
